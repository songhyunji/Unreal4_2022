...
ADodgeballCharacter::ADodgeballCharacter()
{
    ...
    CameraBoom->SetRelativeRotation(FRotator(-70.f, 0.f, 0.f)); // Camera look down player (Don't work...)
	CameraBoom->bUsePawnControlRotation = false; // Not Rotate the arm based on the controller
    // Ignore Pawn's rotation (pitch, yaw, roll)
...
void ADodgeballCharacter::OnDeath_Implementation()
{
    ...
    ADodgeballPlayerController1* PlayerController = Cast<ADodgeballPlayerController1>(GetController());
